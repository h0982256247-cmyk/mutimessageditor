...
import { AppStep, RichMenu, Folder, ProjectStatus } from './types';
...
function App() {
  ...
  const [drafts, setDrafts] = useState<RichMenu[]>([
    { 
      id: '1', 
      name: '夏季促銷活動選單', 
      barText: '查看更多優惠', 
  ...
  const handleSelectDraft = (id: string) => {
    ...
    if (existingDraft) {
        setMenus([existingDraft]);
        setSelectedMenuId(existingDraft.id);
  ...
  const handleSaveDraft = useCallback(() => {
    ...
    if (!mainMenu) return;
    setDrafts(prev => {
        const index = prev.findIndex(d => d.id === mainMenu.id);
        if (index !== -1) {
            const updated = [...prev];
            updated[index] = { ...mainMenu };
            return updated;
        }
        return [{ ...mainMenu }, ...prev];
    });
    setShowSaveSuccess(true);
  }, [menus]);

  const handleUpdateDraftStatus = (id: string, status: ProjectStatus, scheduledAt?: string) => {
    setDrafts(prev => prev.map(d => d.id === id ? { ...d, status, scheduledAt } : d));
