...
import { RichMenu, Folder, ProjectStatus } from '../../../types';
            ...
            {filteredDrafts.map(draft => (
              <Card key={draft.id} className="group cursor-grab active:cursor-grabbing hover:shadow-xl transition-all duration-300 border border-transparent hover:border-primary/20" onClick={() => onSelectDraft(draft.id)}>
                <div draggable onDragStart={(e) => handleDragStart(e, draft.id)} className="aspect-[2500/1686] bg-gray-100 relative overflow-hidden">
                  {draft.imageData ? <img src={draft.imageData} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" alt={draft.name} /> : <div className="w-full h-full flex items-center justify-center text-gray-300 bg-gray-50"><svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg></div>}
                ...
                <div className="p-5">
                  ...
                  <div className="flex justify-between items-center mt-3">
                    <span className="text-[10px] text-secondary bg-gray-100 px-2 py-0.5 rounded uppercase font-bold tracking-tight">{draft.hotspots.length} 熱點</span>
                  </div>
                </div>
              </Card>
            ))}
             ...
             <table className="w-full text-left border-collapse">
                <thead>
                    <tr className="bg-gray-50 border-b border-border">
                        ...
                        <th className="px-6 py-4 text-[11px] font-bold text-secondary uppercase tracking-widest">排程/發布時間</th>
                ...
                <tbody className="divide-y divide-border">
                    {filteredDrafts.map(draft => (
                        <tr key={draft.id} className="hover:bg-primary/5 transition-colors cursor-pointer group" onClick={() => onSelectDraft(draft.id)}>
                            <td className="px-6 py-5">
                                <div className="flex items-center gap-4">
                                    <div className="w-12 h-8 bg-gray-100 rounded border border-border overflow-hidden flex-shrink-0">
                                        {draft.imageData && <img src={draft.imageData} className="w-full h-full object-cover" />}
                                    </div>
                            ...
                            <td className="px-6 py-5 text-sm text-secondary">{draft.scheduledAt || (draft.status === 'published' ? '已發布' : '未設定')}</td>
